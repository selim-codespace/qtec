...
export function ApplyButton({ jobId, jobTitle, company }: ApplyButtonProps) {
    ...
    const handleApply = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Very basic error handling assuming an array of validation errors from our backend Zod logic
            if (Array.isArray(err)) {
                const newErrs: any = {};
                err.forEach((e: any) => { newErrs[e.path[0]] = e.message; });
                setErrors(newErrs);
            } else {
                        ...
                        <div className="p-6">
                            {success ? (
                                <div className="text-center py-10">
                                    <div className="w-16 h-16 bg-success/20 text-success rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">✓</div>
                                ...
                                <form onSubmit={handleApply} className="space-y-4">
                                    {errors.general && <div className="bg-danger/10 text-danger p-3 rounded-md text-sm">{errors.general}</div>}
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-1.5">Resume URL (Portfolio/Drive Link)</label>
                                        <Input
                                            type="url"
                                            placeholder="https://..."
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            className={`flex min-h-[120px] w-full rounded-md border text-sm px-3 py-2 bg-transparent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary ${errors.coverNote ? 'border-danger focus-visible:ring-danger' : 'border-border'}`}
                                            placeholder="Why are you a great fit?"
                                            value={form.coverNote}
                                            onChange={e => setForm({ ...form, coverNote: e.target.value })}
                                            required
                                        />
                                        {errors.coverNote && <span className="text-xs text-danger mt-1 block">{errors.coverNote}</span>}
                                    </div>
